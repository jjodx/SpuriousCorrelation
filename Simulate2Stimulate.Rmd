...
<!-- To do, ajdust the format thing to make sure that I did not reduce the number of digits shown when it was set to 0. Need to use format(round(XX, 2)) if I only want 2 digits -->
...
We will first show that Spearman correlation is again unable to solve the problem of subgroups eventhough it slightly reduces the problem. Then we will show that one needs to take the subgroups into account in a regression model in order to bring the percentage of false-positive correlations back to 5% (which is expected given that the two underlying variables are uncorrelated, r=`r Rval`)
