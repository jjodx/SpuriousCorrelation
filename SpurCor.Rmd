...
14. use ggarrange more
15. zoom on p-values below 0.05??
...
BinSize <- 0.05 # for histograms of p-values
```
...
PPCI_50 <- ggplot(res, aes(x=CI, fill=Sig)) + 
    geom_histogram(aes(x=CI,fill=Sig),binwidth=BinSize,boundary = 0.05,color="black") +
    scale_fill_manual(values = Fcolor) +
    theme(legend.position='none')+
    scale_y_continuous(expand = c(0.1,0)) +
...
As a correlate of this demonstration, it is interesting to investigate what the probability that the correlation of a second experiment will fall into the CI of the correlation of a first experiment. For instance, the CI for the correlation presented in  was . 
What is the chance that a second experiment with the same sample size provides a correlation that falls in this CI. To simulate this case;However, it is interesting to My question was the following: If we perform an experiment once and compute the 95% confidence interval of the mean (e.g. [0.1 to 0.4]). What is the probability that, if we repeat the experiment, the new sample mean will fall within the previously computed confidence interval (see Matlab code below).
(see panel B of`r fig_nums("PPCIboth",display="cite")`)
...
Plot2B <- ggplot(Good2B5, aes(x=NSig)) + 
    geom_label(aes(x = 1, y=Yall, label = paste(Nall, "%")),
               ...
               size = 3,
               fill = NA)+
    geom_histogram(binwidth=BinSize,boundary = 0.05,color="black") +
    coord_cartesian(xlim = c(0, 1))+
    theme(legend.position='none')+
    scale_y_continuous(expand = c(0.1,0)) +
...
## Understanding p-values: final assignments
...
To generate subgroups, we first generate the x and y parameters for the `r PopSize` individuals. We will then split the population into two subgroups and add a value of `r SDdistSub` to each the values of both varialbes for one of the two subgroups. By doing so, one obtains the following grahics.
