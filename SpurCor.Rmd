...
Simulating such correlation repeatedly can allow one to look at the distribution of p-values when no correlation is present in the underlying variables. As described in many other papers [@benjamini1995controlling], this distribution if completely flat and 5% of the p-values are below the typical 5% significance threshold. 
...
If we look at the mean correlation from only significant studies, it is always larger than the actual population average correlation. With low power, significant effect size are always inflated .
