...
11. illustrate false positive with graphics (bar plot with blue and orange rectangles)
...
d=data.frame(x1=c(0.33,0.33), x2=c(0.66,0.66), y1=c(0,Psig), y2=c(Psig,1), t=c('a','b'))
  PP3_0 <- ggplot() +  
    ...
    geom_segment(aes(x = 0.75, y = 0, xend = 0.75, yend = Psig),
...
It is also interesting to simulate cases where the correlation is not zero. While simulating correlations with R=0 thaught us about false-positive (type I error), the importance of sample size and the multiple comparison problem, simulating corelation with $R\ne0$ can teach us about power (and type II error), the precision of estimation and the consequences of publishing only significant studies.
...
`r fig_nums("PP2_50",display="CITE")` also shows that non-significant p-values cannot be considered as proof that there are no correlation between the variables x and y. All the non-significant correlations on the graph above are false-negative (they are not significant even though x and y are correlated). False-negative correlations correspond to type II error. These correlations are non-significant while the sampled variables x and y actually are. The type II error is linked to the power of an experiment.
...
d=data.frame(x1=c(0.33,0.33), x2=c(0.66,0.66), y1=c(0,Psig), y2=c(Psig,1), t=c('a','b'))
  PP3_50 <- ggplot() +  
    ...
    scale_fill_manual(values = Fcolor, 
                      ...
                      labels=c("p<=0.05", "p>0.05")) +
    geom_hline(yintercept=0.05, linetype="dashed", color = "red") +
    geom_segment(aes(x = 0.75, y = 1, xend = 0.75, yend = Psig),
     lineend = 'butt', linejoin = 'bevel',
     size = 0.5, arrow = arrow(length = unit(0.2, "inches")))+
    geom_label(aes(x = 0.8, y = 1-Psig/2, label = "False-Negative"), 
               hjust = "center", 
               vjust = "bottom", 
               colour = "black", 
               label.size = NA, 
...
The problem with power is that we rarely now in advance how big the effect size will be.
