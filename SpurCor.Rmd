...
10. Simulate too good to be true? if modest power, we should have a mix of significant and non-significant studies. Or is this too much??? --> easy with r=0.5 and N=15
...
Statistical significance is at the heart of a philosophical debate. Should we abandon the use of p-values, should we use a more severe significance threshold or should we use an adapted one? Is the Bayes Factor better suited for science than the p-values. These questions are often hotly debated in scientific papers or on social media. Yet, the problem with p-values is that people do not understand them properly or do not understand the circumstances under which a p-value can be used. Furthermore, many people do not have a lot of insights how p-values behave in different circumstances. Here, we use our ability to randomly generate correlated or uncorrelated variables as a tool to get insights into p-values linked to correlations and many problems that have been discussed in science.

But p-values on their own have very little value if they are not coupled to the effect size of an effect. This part is linked to publication bias. That is, people do not cherish not significant effect and rarely published them while significant effects are published very easily. We will thus use simulation to understand how publication bias 

Correlations were chosen because they are very frequently used in scientific papers. Yet, as we will see below, a slight irregularity in the data is sufficient to obtain significant but spurious correlations.
...
Being able to perform these simulations in any programming language has certainly a large added value. The interested reader can use the code below to perform those simulations in R. For the readers who do not have time or willingness to do so, they can use the web application developped for this paper (See ShinyApp section below).
...
In contrast, the probability of observing only significant p-values decreases with the number of experiments considered in one series. For instance, when considering series of `r Ngood_10` experiments, the probability of observing `r Ngood_10` significant p-values is only `r Nall28_10`%, despite having 80% power. Again, this reinforces the impression that every scientists should now and then report experiments that failed. Yet, we know this is rarely the case.
