...
6) Fig number for reference and fig legends
...
A first solution is to use a non-parametric rank correlation such as Spearman. Here, the graph below shows that the influence of the outlier is much reduced and the type I error rate (= `r PsigSpear`) is very close to the 5% level.
``` {r echo=FALSE}
PP3Spear
```
...
In contrast, the use of winsorized correlation solves the problem as robust correlation methods are designed to handle outliers. This is confirmed by the graph below, which shows that the influence of the outlier is eliminated and the type I error rate (= `r PsigRob`) is very close to the 5% level.
``` {r echo=FALSE}
PP3Rob
```
