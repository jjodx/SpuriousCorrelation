...
5) Fig number for reference and fig legends
...
````{r, echo=FALSE, fig.cap= fig_nums("PP4")} 
PP4
```
...
```{r fig2, echo=FALSE, fig.cap=fig_nums("PP2")} 
PP2
```
...
```{r, echo=FALSE, fig.cap=fig_nums("PP1")} 
PP1
```
...
``` {r PP175, echo=FALSE, fig.cap=fig_nums("PP175")}
PP175
```
...
A first solution is to use a non-parametric rank correlation such as Spearman (DEFINE). Here, the graph below shows that the influence of the outlier is much reduced and the type I error rate (= `r PsigSpear`) is not very close to the 5% level.
``` {r echo=FALSE}
PP3Spear
```
...
Note that for `r format(PsigNDBC, digit=2)`% of these significant correlations, the between groups difference was not significant. Yet, these subgroups yielded a positive correlation.

In addition, the bigger the groups get, the smaller the difference between the subgroups must be to give rise to a significant correlation. For instance, for a sample of `r PopSize`, a difference of `r SDdistSub75` standard deviation between group does not inflate the type I error rate (`r PsigSub75_15`% of correlations are significant). However, this difference becomes important for larger populations. For instance, a difference of `r SDdistSub75` increase the type I error rate to `r PsigSub75` and this number increases with sample size.
