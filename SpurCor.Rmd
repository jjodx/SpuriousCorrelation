...
3. make choise between simulation tutorial and statistics tutorial
4. link with ShinyApp
5. For fig8, add arrows showing Type II error, power and type I error. This is the first such representation.
6. add cross-references to Figure (remove below and above)
7. replace R code by latex notation $i$
8. add comments to code
9. make assignments for each section
10. Simulate too good to be true? if modest power, we should have a mix of significant and non-significant studies. Or is this too much???
...
Refer to lakens on ameliorating the interpretation of p-values

# Learning about statistics from simulated correlations.

```{r ParamDef, echo=FALSE}
PopSize=15
Nsim=50 # should be 1000
Rval=0
Rval50=0.5
```
...
These values are unitless because they have been normalized. Some of them are positive other negative values.

Now that we have generated data for height and working memory capacity for each individual, we can test whether these two parameters are correlated. We wil first use the Pearson's correlation, which is used in most scientific studies.
Off course, one should not expect any correlation between height and working memory capacity as these are sampled from random independent distributions. The two randomly generated datasets have a correlation coefficient R= `r format(R$r[1,2], digits = 2)` (p-value = `r format(R$P[1,2], digits = 2)`). Given the process used for generated this data, we indeed expected a low correlation and a non-significant p-values as there is not link between them.
````{r plotR0, echo=FALSE, fig.cap= fig_nums("PP4")} 
PP4
```

Yet, it is also possible that some of these correlations between randomly generated data will be significant. Such significant correlations between randomly generated parameters are called false-positive. That is, the correlation between the parameters samples in `r PopSize` individuals suggest that the two variables (height and working memory capacity) are correlated despite them being not.

## simulating multiple correlation between two randomly generated samples

### doing that in R

One can achieve that in R by using the `mvrnorm` function from the MASS package. To draw `r PopSize` samples from a normal distribution, we can do
``` {r eval=FALSE}
library(Hmisc) # for rcorr
library(MASS) # for mvnorm
set.seed(12)# to make sure everybody gets the same results
...
For the readers that are not (yet) familiar with the R language, one can simply follow the three following steps to be able to interactively follow this tutorial:
...
Simulating such correlation repeatedly can allow one to look at the distribution of p-values when no correlation is present in the underlying variables. As described in many other papers, this distribution if completely flat and 5% of the p-values are below the typical 5% significance threshold. 
...
It is also interesting to simulate cases where the correlation is not zero. While simulating correlations with R=0 thaught us about false-positive (type I error), the importance of sample size and the multiple comparison problem, simulating corelation with $R<>0$ can teach us about power (and type II error), the precision of estimation and the consequences of publishing only significant studies.
...
The generated values can be seen in the table below. One can see that individuals that appear to have a larger height also have a larger weight.

```{r DisplayMatrixR50, echo = FALSE, results = 'asis'}
kable(XY, caption = "randomly generated value for the individuals of the population")
```

This is even more apparent on  `r fig_nums("PP450", display = "cite")` where the relationship between height and weight is visible.

```{r plotCorrelR50, echo=FALSE, fig.cap=fig_nums("PP450")}
PP450
```
Together, these two variables are correlated with a correlation coefficient R= `r format(R$r[1,2], digits = 3)` (p-value = `r format(R$P[1,2], digits=3)`). Off course, the correlation between these two variables is not exactly equal to the simulated value (R= `r Rval50`) We will see later how we can recover the actual simulated correlation. 
...
This graph also shows that non-significant p-values cannot be considered as proof that there are no correlation between the variables x and y. All the non-significant correlations on the graph above are false-negative (they are not significant even though x and y are correlated).
...
$$ y = a*x + c*G $$ where $G$ is equal to zero for one subgroup and one for the other. This equation makes the hypothesis that we expect the same correlation for both groups given that there are no interaction term between $x$ and $G$. 
...
if we follow the same line of reasoning, one can investigate the influence of outlier or subgroups on t-test. The results should be the same if one follows Bishop's blog.
...
2) if there is a correlation, simulations

Simulations can be useful to test potential solution to known problems.