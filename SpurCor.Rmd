...
Now, we will repeat this process a number of times in order to obtain the distribution of the p-values when there is a correlation R=0.5. 
...
As indicated on this graph, the expected power of an experiment is 80% (or 90%). Yet, this is here not the case. To detect a correlation R=0.5, one needs `r PopSize` observations. In this case, the percentage of significant correlations reaches the power requirement of 80%.
``` {r echo=FALSE}
PP28
```

This manipulations demonstrate how one can calculate the required sample size for detecting a correlation of a given value.
